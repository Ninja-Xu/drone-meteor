...
var CanvasWindy = function (json,params) {
    ...
    this.isdestroy = false;//是否销毁，进行删除操作
    this._init();
};
CanvasWindy.prototype = {
    ...
    removeLines: function () {
        ...
        this.isdestroy = true;
        this.canvas.width = 1;
        document.querySelector('.viewer').removeChild(this.canvas);
    },
    //根据粒子当前所处的位置(棋盘网格位置)，计算经纬度，在根据经纬度返回屏幕坐标
    _tomap: function (lng,lat,particle) {
        var ct3 = this.Cesium.Cartesian3.fromDegrees(lng,lat,0);
        // 判断当前点是否在地球可见端
        var isVisible = new this.Cesium.EllipsoidalOccluder(this.Cesium.Ellipsoid.WGS84, this.viewer.camera.position).isPointVisible(ct3);
        var pos = this.Cesium.SceneTransforms.wgs84ToWindowCoordinates(this.viewer.scene, ct3);
        if(!isVisible){
            particle.age = 0;
        }
        // console.log(pos);
    ...
    randomParticle: function (particle) {
        var safe = 2,x=-1, y=-1,lng=null,lat=null;
        ...
        do {
            try{
                if(hasextent){
                    ...
                    var cartesian = this.viewer.camera.pickEllipsoid(new this.Cesium.Cartesian2(pos_x, pos_y), this.viewer.scene.globe.ellipsoid);
        ...
        } while (this.windField.getIn(x, y)[2] <= 0 && safe++ < 2);
        var field = this.windField;
